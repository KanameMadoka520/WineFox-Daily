...
const AFFINITY_LEVEL_NAMES = ['陌生人', '初识', '点头之交', '熟人', '好朋友', '密友', '挚友', '知己', '灵魂伴侣', '命运之绊']
 ...
 * @param {{ hasPuppeteer?: Function, renderAnalyticsCard?: Function, renderImageFeature?: Function, finalConfig?: object, logger?: object }} [options]
 */
function registerAnalyticsCommands(ctx, affinitySystem, getTodayPassiveCount, options = {}) {
  const { hasPuppeteer, renderAnalyticsCard, renderImageFeature, finalConfig = {}, logger = console } = options
  ...
  ctx.command('酒狐统计', '查看群内酒狐互动健康表与好感度分布')
    .action(async ({ session }) => {
      ...
      if (!renderImageFeature || !renderAnalyticsCard || !hasPuppeteer) return textOutput

      return renderImageFeature({
        feature: '酒狐统计',
        imageKey: 'imageAnalytics',
        imageEnabled: finalConfig.imageAnalytics,
        textOutput,
        session,
        render: () => renderAnalyticsCard(ctx, { data }),
        fallbackMessage: '酒狐悄悄话: 统计卡片生成失败了，请稍后再试一次...',
      })
    })
}
