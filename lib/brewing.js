...
class BrewingSystem {
  ...
  getRecipeList() {
    ...
    for (const recipe of RECIPES) {
      ...
      lines.push(`  消耗狐狐券: ${recipe.cost}张`)
   ...
   * @param {string} recipeName
   */
  async confirmBrewing(userId, recipeName, timeReduction = 0, qualityBoost = 0) {
    const userData = this._getUserData(userId)
    ...
    let qualityMin = recipe.qualityMin
    let qualityMax = recipe.qualityMax
    qualityBoost = Math.max(0, Math.min(1, Number(qualityBoost || 0)))
    if (qualityBoost > 0 && Math.random() < qualityBoost) {
      qualityMin = Math.min(QUALITIES.length - 1, qualityMin + 1)
      qualityMax = Math.min(QUALITIES.length - 1, qualityMax + 1)
    }
    userData.brewing = {
      recipe: recipe.name,
      startTime: now,
  ...
  async finishNow(userId) {
    const userData = this._getUserData(userId)
    if (!userData.brewing) return false
    userData.brewing.finishTime = Date.now()
    await this._save()
    return true
  }
