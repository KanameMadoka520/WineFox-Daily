...
function registerPassive(ctx, quotesLoader, affinity, config) {
  ...
  if (config.enablePassiveKeyword !== false) {
    ...
    ctx.middleware((session, next) => {
      ...
      logger.debug(`[fox] 关键词检测: 命中=${hits.map(h => h.keyword).join('/')} → 选中「${pick.keyword}」(${(pick.triggerConfig.chance * 100).toFixed(0)}%), 群=${guildId}`)

      if (!rollChance(pick.triggerConfig.chance)) {
        logger.debug(`[fox] 关键词「${pick.keyword}」概率未通过, 跳过`)
        return next()
      }
      ...
      if (matched.length === 0) {
        logger.debug(`[fox] 关键词「${pick.keyword}」概率通过但语录库无匹配, search=${pick.triggerConfig.search.join('/')}`)
        return next()
      }
