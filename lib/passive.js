...
function registerPassive(ctx, quotesLoader, affinity, config) {
  ...
  if (config.enablePassiveKeyword !== false) {
    ...
    ctx.middleware((session, next) => {
      ...
      if (typeof config.isPassiveKeywordAllowed === 'function' && !config.isPassiveKeywordAllowed(session)) {
        return next()
      }
      ...
      const passiveChanceBonus = session.userId && config.getPassiveChanceBonus ? config.getPassiveChanceBonus(session.userId) : 0
      const finalChance = Math.min(1, pick.triggerConfig.chance + passiveChanceBonus)
      logger.info(`[fox] 关键词检测: 命中=${hits.map(h => h.keyword).join('/')} → 选中「${pick.keyword}」(${(finalChance * 100).toFixed(0)}%), 群=${guildId}`)
