...
class FavoritesSystem {
  ...
  listFavorites(userId, page = 1) {
    const data = this.getFavoritesData(userId, page)
    ...
    const lines = [
      `== 酒狐收藏夹 (${data.page}/${data.totalPages}页) ==`,
      `共 ${data.totalCount} 条收藏`,
      '',
    ]

    for (const item of data.items) {
      lines.push(`${item.index}. ${item.preview}`)
    }
  ...
  getFavoritesData(userId, page = 1) {
    const userData = this._getUserData(userId)
    const quotes = Array.isArray(userData.quotes) ? userData.quotes : []
    ...
    const end = Math.min(start + perPage, quotes.length)
    ...
    for (let i = start; i < end; i++) {
      ...
      items.push({
        index: i + 1,
        quote,
        preview: quote.length > 52 ? quote.substring(0, 52) + '...' : quote,
      })
    }
