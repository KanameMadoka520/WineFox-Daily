...
class CommissionSystem {
  constructor(memoryDir, logger, affinity, config = {}) {
  ...
  async recordProgress(userId, eventType, value = 1) {
    ...
    for (const task of userData.tasks || []) {
      ...
      if (task.progress >= task.target) {
        task.completed = true
        const bonusRate = Math.max(0, Number(this.config.getCommissionBonus ? this.config.getCommissionBonus(userId) : 0))
        const finalReward = task.reward + Math.floor(task.reward * bonusRate)
        await this.affinity.addTickets(userId, finalReward)
        completedTasks.push({ ...task, actualReward: finalReward })
      }
    }
