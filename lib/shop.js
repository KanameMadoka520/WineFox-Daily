...
class ShopSystem {
  ...
  _getUserData(userId) {
    ...
    if (!this.data[userId].tempEffects) {
      this.data[userId].tempEffects = {}
    }
  ...
  async equip(userId, itemName) {
    ...
    return { success: true, message: shopItem?.equipLine || '酒狐悄悄话: 装备好了！', itemId: item.id }
  }
  ...
  getTempEffect(userId, key) {
    const userData = this._getUserData(userId)
    return userData.tempEffects[key]
  }

  async setTempEffect(userId, key, value) {
    const userData = this._getUserData(userId)
    userData.tempEffects[key] = value
    await this._save()
  }

  async clearTempEffect(userId, key) {
    const userData = this._getUserData(userId)
    delete userData.tempEffects[key]
    await this._save()
  }

  async takeTempEffect(userId, key) {
    const userData = this._getUserData(userId)
    const value = userData.tempEffects[key]
    delete userData.tempEffects[key]
    await this._save()
    return value
  }

  async decrementTempEffect(userId, key, amount = 1) {
    const userData = this._getUserData(userId)
    const current = Number(userData.tempEffects[key] || 0)
    if (current <= 0) return 0
    const next = Math.max(0, current - amount)
    if (next > 0) userData.tempEffects[key] = next
    else delete userData.tempEffects[key]
    await this._save()
    return next
  }
}
