...
class DailyQuote {
  ...
  constructor(memoryDir, logger) {
    ...
    /** @type {{ date: string, quote: string }} */
    this.daily = { date: '', quote: '' }
    /** @type {string[]} */
    this.recentHistory = []
    this._load()
  }
...
// 移除 async 和 withLock，改为同步读取
   ...
   */
  async getTodayQuote(allQuotes) {
    const today = getTodayKey()

    if (this.daily.date === today && this.daily.quote) {
    ...
    this.daily = { date: today, quote: picked || '主人，今天的语录本好像被风吹走了...' }
