...
function estimateHelpGroupHeight(group) {
  const items = Array.isArray(group?.items) ? group.items : []
  const rowWeight = group?.dense ? 0.82 : group?.compact ? 0.72 : 1
  return Math.round(78 + items.length * 28 * rowWeight)
}

function buildHelpColumns(groups, columnCount = 2) {
  const safeGroups = Array.isArray(groups) ? groups : []
  const count = Math.max(1, Math.min(columnCount, safeGroups.length || 1))
  if (count <= 1 || safeGroups.length <= 1) return [safeGroups]
  if (count >= safeGroups.length) return safeGroups.map(group => [group])

  const heights = safeGroups.map(group => estimateHelpGroupHeight(group))

  function splitSequential(startIndex, remainingColumns) {
    const remainingGroups = safeGroups.slice(startIndex)
    if (remainingColumns <= 1 || remainingGroups.length <= 1) return [remainingGroups]

    const remainingHeights = heights.slice(startIndex)
    const totalHeight = remainingHeights.reduce((sum, value) => sum + value, 0)
    const desiredFirst = totalHeight / remainingColumns
    const maxSplit = remainingGroups.length - (remainingColumns - 1)
    ...
    for (let i = 1; i <= maxSplit; i++) {
      prefixHeight += remainingHeights[i - 1]
      const diff = Math.abs(prefixHeight - desiredFirst)
      if (diff <= bestDiff) {
        bestDiff = diff
        bestSplitIndex = i
      }
    ...
    return [
      remainingGroups.slice(0, bestSplitIndex),
      ...splitSequential(startIndex + bestSplitIndex, remainingColumns - 1),
    ]
  }

  return splitSequential(0, count)
}
...
${themeVarsCss}
        ...
        * { box-sizing: border-box; }
        ...
        .help-row {
          ...
          background: transparent;
          border: 0;
          border-top: 1px solid rgba(183, 209, 223, 0.48);
          border-radius: 0;
        }
        ...
        .help-admin-group .help-items {
          display: grid;
          gap: 8px 12px;
        }
        .help-admin-group .help-row {
          padding: 7px 9px;
          border: 1px solid rgba(183, 209, 223, 0.56);
          border-radius: 10px;
          background: rgba(255,255,255,0.82);
        }
        .help-admin-group .help-row:first-child {
          padding-top: 8px;
          border-top: 1px solid rgba(183, 209, 223, 0.56);
        }
        .help-footnote {
          margin-top: 8px;
          padding: 0 2px;
...
function renderHelpCard(ctx, payload) {
  ...
  const adminGroup = groups.find(group => group?.title === '管理员') || null
  const primaryGroups = adminGroup ? groups.filter(group => group !== adminGroup) : groups
  const primaryColumnCount = primaryGroups.length >= 6 ? 3 : primaryGroups.length > 1 ? 2 : 1
  const helpColumns = buildHelpColumns(primaryGroups, primaryColumnCount)
  const adminColumnCount = adminGroup?.items?.length >= 12 ? 3 : 2
  const cardWidth = primaryColumnCount >= 3 ? 760 : primaryColumnCount === 2 || adminGroup ? 720 : 660
  const status = data.status || {}
  ...
  const renderHelpGroup = (group, options = {}) => {
    const items = Array.isArray(group?.items) ? group.items : []
    const itemLayoutStyle = options.admin
      ? ` style="grid-template-columns: repeat(${Math.max(1, options.columnCount || 2)}, minmax(0, 1fr));"`
      : ''
    return `
      <div class="section help-group${group?.compact ? ' compact' : ''}${group?.dense ? ' dense' : ''}${options.admin ? ' help-admin-group' : ''}">
  ...
  const statusSection = (status.season || statusPills.length > 0)
    ...
    `
    : ''
  const columnsSection = helpColumns.length > 0
    ? `<div class="help-columns" style="grid-template-columns: repeat(${helpColumns.length}, minmax(0, 1fr));">${helpColumns.map((column) => `
      <div class="help-column">${column.map(group => renderHelpGroup(group)).join('')}</div>
    `).join('')}</div>`
    : ''
  const adminSection = adminGroup ? renderHelpGroup(adminGroup, { admin: true, columnCount: adminColumnCount }) : ''
  const footerSection = data.footer ? `<div class="help-footnote"><div class="muted">${escapeHtml(data.footer)}</div></div>` : ''
  const content = `${statusSection}${columnsSection}${adminSection}${footerSection}`
