...
function buildHelpColumns(groups, columnCount = 2) {
  ...
  if (count === 2) {
    ...
    for (let i = 1; i < safeGroups.length; i++) {
      prefixHeight += heights[i - 1]
      const diff = Math.abs(totalHeight - prefixHeight * 2)
      if (diff < bestDiff) {
        bestDiff = diff
        bestSplitIndex = i
      }
    ...
    return [
      safeGroups.slice(0, bestSplitIndex),
      safeGroups.slice(bestSplitIndex),
    ]
  }
  ...
  for (const group of safeGroups) {
    const estimatedHeight = estimateHelpGroupHeight(group)
    let targetColumn = 0
    for (let i = 1; i < columns.length; i++) {
      if (heights[i] < heights[targetColumn]) targetColumn = i
    }
    columns[targetColumn].push(group)
    heights[targetColumn] += estimatedHeight + 12
  }
...
function renderHelpCard(ctx, payload) {
  ...
  const helpColumns = buildHelpColumns(primaryGroups, primaryGroups.length > 1 ? 2 : 1)
  const cardWidth = primaryGroups.length > 1 || adminGroup ? 780 : 660
  const status = data.status || {}
  ...
  const renderHelpGroup = (group, options = {}) => {
    ...
    return `
      <div class="section help-group${group?.compact ? ' compact' : ''}${group?.dense ? ' dense' : ''}${options.admin ? ' help-admin-group' : ''}">
        ...
        <div class="help-items">${items.map((item) => `
          <div class="help-row">
  ...
  const columnsSection = helpColumns.length > 0
    ? `<div class="help-columns">${helpColumns.map((column) => `
      <div class="help-column">${column.map(group => renderHelpGroup(group)).join('')}</div>
    `).join('')}</div>`
    : ''
  const adminSection = adminGroup ? renderHelpGroup(adminGroup, { admin: true }) : ''
