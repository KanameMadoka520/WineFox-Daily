...
function renderHelpCard(ctx, payload) {
  ...
  const statusSection = (status.season || status.weather || status.mode || status.nextChange)
    ? `
    <div class="section status-hero">
      <div class="status-hero-main">
        <div class="status-season-chip">
        ...
        <div class="status-pill-group">
          ${status.period ? `<span class="pill">时段 · ${escapeHtml(status.period)}</span>` : ''}
          ${status.mode ? `<span class="pill">模式 · ${escapeHtml(status.mode)}</span>` : ''}
        </div>
      </div>
      <div class="status-meta-grid">
        <div class="status-meta-card">
          <div class="label">当前天气</div>
          <div class="status-meta-value">${escapeHtml(status.weather || '未知')}</div>
        </div>
        <div class="status-meta-card">
          <div class="label">下次更替</div>
          <div class="status-meta-value">${escapeHtml(status.nextChange || '未知')}</div>
        </div>
      </div>
    </div>
    `
    : ''
  const content = `${statusSection}<div class="help-rows">${rows.map((row) => `
