...
function estimateHelpItemWeight(item) {
  const label = String(item?.[0] || '')
  const desc = String(item?.[1] || '')
  return 18 + Math.ceil(label.length / 12) * 6 + Math.ceil(desc.length / 18) * 7
}

function estimateHelpGroupWeight(group) {
  const items = Array.isArray(group?.items) ? group.items : []
  const bias = Number(group?.weightBias) || 0
  return 24 + items.reduce((sum, item) => sum + estimateHelpItemWeight(item), 0) + bias
}

function distributeHelpGroups(groups, columnCount = 4) {
  const safeGroups = Array.isArray(groups) ? groups.slice() : []
  const count = Math.max(1, Math.min(columnCount, safeGroups.length || 1))
  const columns = Array.from({ length: count }, () => ({ weight: 0, groups: [] }))

  const sortedGroups = safeGroups
    .map(group => ({ group, weight: estimateHelpGroupWeight(group) }))
    .sort((a, b) => b.weight - a.weight)

  for (const entry of sortedGroups) {
    columns.sort((a, b) => a.weight - b.weight)
    columns[0].groups.push(entry.group)
    columns[0].weight += entry.weight
  }
...
function renderHelpCard(ctx, payload) {
  ...
  const columns = distributeHelpGroups(groups, 4)
  const content = `<div class="help-columns">${columns.map((column) => `
    <div class="help-column">${column.map((group) => `
  ...
  return renderCard(ctx, buildShell('酒狐帮助', escapeHtml(data.title || '指令列表'), content, { cardWidth: 1360 }))
}
