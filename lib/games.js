...
class GamesSystem {
  ...
  playRPS(userChoice) {
    ...
    return {
      ...
      message: `${header}\n\n${line}`,
      affinityBonus: bonus,
      userChoiceName: userName,
      foxChoiceName: foxName,
      flavorLine: line,
    }
  }
  ...
  playGuessNumber(sessionKey, guess) {
    ...
    if (num === session.answer) {
      ...
      const bonus = attempts <= 3 ? 5 : attempts <= 6 ? 3 : 2
      return {
        message: `恭喜主人！答对了！答案就是 ${answer}！\n你用了 ${attempts} 次就猜到了，好厉害！(好感度+${bonus})`,
        finished: true,
        affinityBonus: bonus,
        success: true,
        answer,
        attempts,
        summary: `答案是 ${answer}，你用了 ${attempts} 次就猜中了。`,
      }
    }
