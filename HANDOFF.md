...
> **最后更新**: 2026-03-26
> **当前状态**: v2.3 主线已落地；v2.3.1 已合入稳定性/运维增强（原子写与坏档备份、图片渲染队列与缓存、玩家存档备份/恢复、酒狐账本、渲染统计/渲染诊断）+ 玩法扩展（动态商店：今日特供/折扣/限购），待宿主环境联调
> **版本号**: 2.3.1
...
| `lib/card-renderer.js` | 全部图片卡片渲染器 | 帮助卡已改为紧凑多列布局；商店布局已优化 |
...
- `酒狐帮助`：轻量状态条 + 分组全宽 section；各分组内部按 3~5 列压缩展示，目标是降低聊天窗口内的总高度
