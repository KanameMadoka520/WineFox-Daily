    ...
    return {
      ...
      groups: [
        ...
        },
        {
          ...
          items: [
            ['酒狐UI [主题名]', '查看或切换图片主题'],
            ['酒狐渲染诊断 [-i]', '渲染链路/队列/缓存/统计摘要/存档健康'],
            ['酒狐渲染统计 [N]', '查看最近 N 次渲染统计（-c 清空）'],
            ['酒狐渲染自检', '验证 Puppeteer 生图链路'],
            ['酒狐缓存清理', '清理图片渲染缓存（仅内存）'],
            ['酒狐存档备份', '备份玩家存档（仅 opsAdminIds）'],
            ['酒狐存档列表 [N]', '查看存档备份列表（仅 opsAdminIds）'],
            ['酒狐存档清理 [N]', '清理旧备份，只保留最近 N 个（仅 opsAdminIds）'],
            ['酒狐存档恢复 <id> -f', '从备份恢复（危险，仅 opsAdminIds）'],
            ['酒狐账本 @某人', '查询他人账本（仅 opsAdminIds）'],
            ['酒狐季节状态', '查看当前季节干预状态'],
            ['酒狐季节设置 <季节>', '手动切换循环季节'],
